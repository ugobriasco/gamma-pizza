...
validate.logout = request => {
  // The email should be a string and it should match a given pattern
  const email =
    typeof request.payload.email.trim() == "string" &&
    validateEmail(request.payload.email.trim())
      ? request.payload.email.trim()
      : false;

  // The auth token shall be included in the header and be a string
  const authToken =
    typeof request.headers.auth == "string" && request.headers.auth.length > 0
      ? authToken
      : false;
