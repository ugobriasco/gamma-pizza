...
token.extend = email =>
  _data
    ...
    .then(extendedToken => {
      // If the token to be updated is inconsistant, then delete it (prevents anomalies under stress test)
      if (!extendedToken.id) {
        return token.delete(email);
      } else {
        // If all good, update the token
        return _data.pUpdate("tokens", email, extendedToken);
      }
    });
