    ...
    .then(req => {
      ...
      if (req.isVerified === false) {
        ...
        chosenHandler(req, response => {
          const { statusCode, data, contentType } = response;
          ...
          if (contentType == "html") {
            // Set headers to text/html if requested
            res.setHeader("Content-Type", "text/html");
            res.writeHead(statusCode);
            const payloadString = typeof data == "string" ? data : "";
            res.end(payloadString);
          } else {
            //Send default response as application/json
            res.setHeader("Content-Type", "application/json");
            res.writeHead(statusCode);
