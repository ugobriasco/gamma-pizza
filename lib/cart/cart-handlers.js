...
cart.postCart = (req, res) => {
  ...
  const { email, items, isValid } = validate.cart(req.payload);
  ...
  if (!isValid) {
    ...
    Promise.all([getUser(email), getCart(email)])
      .then(promises => {
...
cart.updateCart = (req, res) => {
  ...
  if (!isValid) {
    ...
    Promise.all([getUser(email), getCart(email)]).then(promises => {
